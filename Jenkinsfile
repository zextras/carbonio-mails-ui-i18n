...
def createBuild(sign) {
	...
	if (sign) {
		dir("artifact-deployer") {
			...
			sh(script: """#!/bin/bash
				./sign-zextras-zip ../pkg/com_zextras_zapp_mails.zip
			""")
		}
	}
}
...
pipeline {
	...
	environment {
		...
		TRANSLATIONS_REPOSITORY_NAME = "zextras/com_zextras_zapp_mails"
	}
	stages {
		...
		stage("Release automation") {
			...
			steps {
				...
				createBuild(true)
				archiveArtifacts(
					artifacts: "pkg/com_zextras_zapp_mails.zip",
					fingerprint: true
				)
				stash(
					includes: "pkg/com_zextras_zapp_mails.zip",
					name: 'zimlet_package'
				)
				...
				script {
					...
					doc.upload(
						file: "docs/website/build/com_zextras_zapp_mails/**",
						destination: "iris/zapp-mails/$BRANCH_NAME"
					)
				}
